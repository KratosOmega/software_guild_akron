...
$(document).ready(function () {
    refreshAddressTable();
//    //********************************************************* drop down list - do this later
//    populateCountries("add-country", "add-state");
//    populateStates("add-country", "add-state");
//    //********************************************************* drop down list - do this later
    ...
    $.each(data, function (key, value) {
        contentTable.append($("<tr>")
                .append($("<td>")
                        .append($("<a>")
                                .attr({
                                    'data-address-id': value.addressId,
                                    'data-toggle': 'modal',
                                    'data-target': '#detailsModal'
                                }).text(value.firstName + " " + value.lastName)))
                .append($("<td>").text(value.city + ", " + value.state + ", " + value.country))
                .append($("<td>")
                        .append($("<button>")
                                .attr({
                ...
                .append($('<td>')
                        .append($('<button>')
                                .attr({
                                    'class': 'btn btn-primary',
                                    'onClick': 'deleteAddress(' + value.addressId + ')'
                                })
                                .text('Delete'))));
    });
}
...
function deleteAddress(id) {
    var answer = confirm('Do you really want to delete this address record?');
...
$('#detailsModal').on('show.bs.modal', function (event) {
    ...
    var id = element.data('address-id');
    var modal = $(this); //-------------------------------------------------------------- what is "this" represent?
    // Get our object via AJAX
    $.ajax({
        type: 'GET',
        url: 'address/' + id
    }).success(function (address) {
        modal.find('#details-address-id').text(address.addressId);
        modal.find('#details-address-name').text(address.firstName + ", " + address.lastName);
        modal.find('#details-address-address').text(address.houseNumber + " " + address.street + " " + address.city + ", " + address.state + ", " + address.zipCode + " " + address.country);
    });
});
...
$('#editModal').on('show.bs.modal', function (event) {
    ...
    $.ajax({
        ...
        modal.find('#edit-address-firstName').val(address.firstName);
        modal.find('#edit-address-lastName').val(address.lastName);
        modal.find('#edit-address-houseNumber').val(address.houseNumber);
        modal.find('#edit-address-street').val(address.street);
        modal.find('#edit-address-city').val(address.city);
        modal.find('#edit-address-state').val(address.state);
        modal.find('#edit-address-zipCode').val(address.zipCode);
        modal.find('#edit-address-country').val(address.country);
    });
});
